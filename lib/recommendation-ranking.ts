...
import { concreteConcepts, lessonRelevance, topicRelevance, topicTerms, type TopicContext } from "./topic-search";
...
export function recommendationFit(resource: Resource, context: RecommendationContext) {
  ...
  const relevance = lessonRelevance(resource.title, evidence, context, resource.provenance?.kind === "web-search" ? resource.reason : undefined);
  ...
  const promotional = /\b(?:best courses|top \d+|roadmap|salary|coupon|discount|sponsored|best .{0,25} courses|courses? .{0,15} ranked)\b/i.test(resource.title) ? 8 : 0;
  const curated = resource.provenance?.kind === "curated" ? 1.5 : 0;
  const score = relevance * 3 + Math.min(concepts.length, 3) * 1.5 + levelFit + teaching
    + (directLesson ? 3 : format === "Channels" ? -4 : 0) + curated - promotional;
...
/** Diversity breaks close ties; it must never promote unrelated material over topic evidence. */
export function rankRecommendations(resources: Resource[], context: RecommendationContext): Resource[] {
  ...
  const candidates = resources.filter(resource => {
    ...
    creator: creatorKey(resource), format: resourceFormat(resource), title: topicTerms(resource.title).join(" ") }));
  ...
  // Sources with independent evidence come first, followed by uncertain and broader matches.
  const tier = (item: typeof candidates[number]) => item.resource.matchContext === "path" ? 2 : item.fit.relevance >= 1 ? 0 : 1;
  for (const group of [0, 1, 2]) {
    const pool = candidates.filter(item => tier(item) === group);
    while (pool.length) {
      const adjusted = (item: typeof pool[number]) => {
        return item.fit.score - Math.min(creators.get(item.creator) || 0, 2) * 3
          - Math.min(formats.get(item.format) || 0, 2) * 2 - Math.min(titles.get(item.title) || 0, 2) * 2
          + Math.min(item.fit.concepts.filter(concept => !covered.has(concept)).length, 2) * 1.5;
      };
      pool.sort((a, b) => adjusted(b) - adjusted(a) || a.index - b.index);
      const { resource, creator, format, title, fit } = pool.shift()!;
      ordered.push(resource);
      fit.concepts.forEach(concept => covered.add(concept));
