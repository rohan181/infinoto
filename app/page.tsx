...
import { initialPath, type LearningPath, type Resource } from "./data";
...
const initial: Workspace = { paths: [{ ...initialPath, source: "example" }], activeId: initialPath.id, completed: {}, saved: [], messages: [], suggestions: [] };
const starters = [
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        const paths = Array.isArray(saved.paths) ? saved.paths.filter(isLearningPath) : [];
