...
import { ArrowDown, ArrowRight, RefreshCw, Settings2, Link2, ArrowUpRight, BookOpen, Bookmark, Check, ChevronDown, FileText, Globe2, GitCompareArrows, GraduationCap, Library, ListVideo, LoaderCircle, Play, Radio, Search, SlidersHorizontal, Sparkles, X } from "lucide-react";
...
export default function TopicResources({ path, topic, saved, onSave, onResources, onSelect, expanded, onExpand }: {
  ...
  useEffect(() => { setCompareResource(null); setQuery(""); setCreator("All creators"); setFocusDraft(""); setCount(6); }, [topic.id]);
  ...
  const base = filterRecommendations(scoped, format, level, query);
  ...
  const existing = all.filter(r => resourceFormat(r) === effectiveFormat);
  const full = all.filter(r => r.type === target.category).length >= 90;
  const activeFocus = focus.topicId === topic.id ? focus.value : "";
  const action = useTopicDiscovery({ pathTitle: path.title, topicTitle: topic.title, description: topic.description, concepts: topic.concepts.slice(0, 6), focus: activeFocus, ...target, level, provider, excludeUrls: [] }, effectiveFormat, ready, `${path.id}:${topic.id}`, additions => onResources(topic.id, additions));
  ...
  return <section className={`recommendation-engine library-refresh ${expanded ? "expanded" : ""}`} aria-label="Content recommendations">
    ...
    <div className="engine-controls">
      ...
      {settings && <div id="library-search-settings" className="library-search-settings">
        ...
        <p>Videos use {videoProvider === "youtube" ? "YouTube" : videoProvider === "exa" ? "Exa" : "Claude"}; blogs and web use {webProvider === "exa" ? "Exa" : "Claude"}. Available choices are remembered.</p>
    ...
    <div className="engine-results" ref={resultsRef} aria-busy={action.busy}>
      ...
      {!resources.length && !action.busy && <div className="engine-empty"><span><Search size={25}/></span><h3>{hasFilters ? "Let’s widen the search." : "A topic worth exploring."}</h3><p>{hasFilters ? "No sources match these filters yet. Try All levels to include videos whose creators haven’t labeled their difficulty." : "No suitable sources found for this topic yet. Refine the search or try another engine."}</p><div><button onClick={() => { if (hasFilters) { setQuery(""); setCreator("All creators"); setLevel("All levels"); } else setSettings(true); }}>{hasFilters ? "Clear filters" : "Refine search"}<ArrowRight size={13}/></button></div></div>}
      ...
      <footer className="engine-footnote"><span><Link2 size={13}/>Direct sources, chosen for this topic</span><p>{provider === "youtube" ? "Difficulty follows labels in video titles. All levels includes unlabeled lessons." : "Difficulty is an estimate of starting knowledge. Some sources may require payment."}</p></footer>
    </div>
